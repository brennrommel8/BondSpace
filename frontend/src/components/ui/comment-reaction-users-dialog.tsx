...
interface CommentReactionUsersDialogProps {
  postId: string;
  commentId: string;
  isOpen: boolean;
  onClose: () => void;
...
export function CommentReactionUsersDialog({ 
  postId, 
  commentId, 
  isOpen, 
  onClose 
}: CommentReactionUsersDialogProps) {
  const { isLoading, error, reactions, reactionsByType, totalCount } = useCommentReactions(postId, commentId);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Comment Reactions ({totalCount})</DialogTitle>
        </DialogHeader>
